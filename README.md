...
A skill vive em `.claude/skills/refactor-arch/` dentro de cada projeto, então basta rodar o Claude Code de dentro da pasta.

### Rodar a skill

A skill executa as Fases 1 e 2 e **pausa** pedindo confirmação antes de escrever qualquer arquivo. Para salvar o relatório, peça na sessão: *"salve o relatório em reports/audit-project-N.md"*.
...
Cada projeto sobe com o seed e as rotas originais respondendo (há `.env.example` com defaults; copiar para `.env` só se quiser trocar segredos). Projetos 1 e 3 usam a porta 5000, rode um de cada vez.

```bash
# code-smells-project (Flask, 5000)
pip install -r requirements.txt && python src/app.py
curl localhost:5000/usuarios      # 200, sem o campo senha

# ecommerce-api-legacy (Express, 3000)
npm install && npm start
curl localhost:3000/api/admin/financial-report   # 401 sem token

# task-manager-api (Flask, 5000)
pip install -r requirements.txt && python seed.py && python app.py
curl localhost:5000/users         # 200, sem o campo password
```

O critério é o mesmo nos três: a aplicação inicia sem erros e os endpoints originais continuam respondendo. As tabelas com todas as rotas conferidas estão na seção **Resultados**.
